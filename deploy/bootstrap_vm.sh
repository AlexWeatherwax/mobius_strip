...
APP_NAME="mobius_strip"REPO_URL="https://github.com/AlexWeatherwax/mobius_strip.git"
...
sudo chown -R "\(DJANGO_USER:\)DJANGO_USER" "$PROJECT_DIR"
if -d "${PROJECT_DIR}/.git" ; then
log "Репозиторий найден, обновляю..."
sudo -u "$DJANGO_USER" -H bash -lc "cd '$PROJECT_DIR' && git fetch --all && git checkout main || true && git pull --ff-only || true"
else
log "Клонирую репозиторий..."
sudo -u "$DJANGO_USER" -H bash -lc "git clone '$REPO_URL' '$PROJECT_DIR'"
fi
}
ensure_venv() {
log "Создаю venv и ставлю зависимости..."
sudo -u "\(DJANGO_USER" -H bash -lc "cd '\)PROJECT_DIR' && python3 -m venv .venv && .venv/bin/pip install --upgrade pip"
if -f "${PROJECT_DIR}/requirements.txt" ; then
sudo -u "$DJANGO_USER" -H bash -lc "cd '$PROJECT_DIR' && .venv/bin/pip install -r requirements.txt"
fi
}
